...
def _cost_of_usage(
    ...
    try:
        prompt_cost, completion_cost = (
            anthropic_cost_per_token(
                model=model.model, usage=usage, service_tier=basis.service_tier, model_info=model_info
            )
            if model.provider == "anthropic"
            else generic_cost_per_token(
                model=model.model,
                usage=usage,
    ...
    except Exception as e:  # noqa: BLE001  # get_model_info raises bare Exception for unmapped models; degrade to an unavailable estimate
        verbose_proxy_logger.debug(
...
def recorded_estimated_autorouter_savings(metadata: Mapping[str, object]) -> float | None:
    ...
    if (
        ...
        or estimate.get("version") not in (1, 2)
