...
import { setTierModelReasoningEffort } from "./complexity_router_tiers";
...
export const ForecastSolverModels = ({ value, onChange, modelOptions, effortOptionsByModel }: Props) => {
  const id = React.useId();
  const names = forecastTierNames(value);
  return (
    <div className="rounded-lg border p-4 space-y-4">
      {names.map((tier, index) => {
        const label = index === 0 ? "Efficient solver" : "Capable solver";
        const models = forecastModels(value.tiers, tier);
        const setModels = (next: string[]) => onChange({ ...value, tiers: { ...value.tiers, [tier]: next } });
        return (
...
const ForecastClassifierConfig = ({ value, onChange, modelOptions, effortOptionsByModel }: Props) => {
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      <div className="space-y-1">
        <Label htmlFor={`${id}-judge`}>Classifier Model</Label>
        <SearchSelect
          inputId={`${id}-judge`}
          ...
          onValueChange={(model) => onChange({ ...value, classifier_llm_config: { ...llm, model: model ?? "" } })}
        />
      </div>
