...
def _sse(completed: bool = True, model: str = "claude-sonnet-5") -> tuple[bytes, ...]:
    events: Final = (
        {  # mutable-ok: json.dumps needs a concrete event dictionary
            "type": "message_start",
            "message": {**_START_MESSAGE, "model": model},  # mutable-ok: JSON message copy supplies the chosen model
        },
        *_EVENTS[1:],
    )
    return tuple(
...
def _upstream(request: httpx.Request) -> httpx.Response:
    ...
    if body.get("stream") is True:
        return httpx.Response(
            200,
            content=b"".join(_sse(model=model)),
            headers=MappingProxyType({"content-type": "text/event-stream"}),
            request=request,
        )
    return httpx.Response(
        200,
        json={**_COMPLETED, "model": model},  # mutable-ok: httpx JSON encoding needs a concrete dictionary
        request=request,
    )
