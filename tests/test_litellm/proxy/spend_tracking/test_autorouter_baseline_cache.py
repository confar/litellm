...
async def test_lost_unpublished_repairs_preserve_pending_uncertainty(rig: _Rig, loss: str) -> None:
    ...
    if loss == "restart":
        ...
        new_scope: Final = rig.prepare("new-owner", session="new-scope")
        assert await rig.estimator.reserve(new_scope) is None
        assert (await rig.finish(new_scope, observed_usage=_observed())).provenance != "observed_initial"
...
async def test_storage_deadline_bounds_all_conflict_attempts_and_preserves_cancellation(rig: _Rig) -> None:
    class SlowConflicts(InMemoryBaselineStore):
    ...
    estimator: Final = BaselineCacheEstimator(SlowConflicts(), rig.clock)
    initial: Final = rig.prepare("deadline")
    started: Final = time.monotonic()
    result: Final = await estimator.reserve(initial)
    assert result is not None and result.reason == "state_unavailable"
    assert time.monotonic() - started < 0.5
    assert estimator.repairs
    cancelled: Final = asyncio.create_task(estimator.reserve(rig.prepare("cancelled")))
